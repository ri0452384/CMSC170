...
def k_means(points,centroids,max_iteration):
    ...
    for c in classes:
        centroid_assignments.append(c[1])
...
"""returns ONLY the ff: [0]centroid assignments and [1] 16 centroid coordinates"""
ans = solve(image_values)
print(ans)
new_image = Image.new('RGB', (128, 128))
new_image.putdata(ans)
""" same step still generates 10KB image file.
for y in range(0,127):
    for x in range(0,127):
        element = int(ans[0][128*y + x])
        new_color = (ans[1][element])
        new_image.putpixel((x,y),new_color)
        """
new_image.save("kmimg-new.png")
